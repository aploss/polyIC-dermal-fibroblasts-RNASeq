---
title: "Expanded_Design_Factor_SpeciesHomologs_Outputs_SpeciesMapped_Comparisons"
output: pdf_document
---
...
##Purpose
To compare the DGE profiles created in "Expanded_Design_Factor_SpeciesHomologs_Outputs_SpeciesMapped". 

```{r files}
##counts from reads that were aligned to the species-specific genome
DGE_source <- file.path("Expanded_Design_Factor_SpeciesHomologs_Outputs_SpeciesMapped")
DGE_source_names <- Sys.glob(file.path(DGE_source, "*vs_human_dds1 DGE_results.txt"))

DGE_readin <- lapply(DGE_source_names, function(x) read.delim(x)) 
names(DGE_readin) <- DGE_source_names %>% str_replace("Expanded_Design_Factor_SpeciesHomologs_Outputs_SpeciesMapped/", "") %>%
  sub('\\d+-\\d+-\\d+\\s', '', .) %>%
...
##All the values are negative, indicating these genes' expression is less than what is observed in human

zoo <- llply(DGEs_filtered, function(x) dplyr::select(x, X)) 
names(zoo) <- c("bonobo", "chimp", "gorilla", "baboon", "orangutan", "pigtailed", "rhesus", "squirrel_monkey")

zoo_log2FC <- llply(DGEs_filtered, function(x) dplyr::select(x, X, log2FoldChange)) 
names(zoo_log2FC) <- c("bonobo", "chimp", "gorilla", "baboon", "orangutan", "pigtailed", "rhesus", "squirrel_monkey")
...
zoo_df <- zoo_df[c("X", "chimp", "bonobo", "gorilla", "orangutan", "baboon", "rhesus", "pigtailed", "squirrel_monkey")]

zoo_avg_df <- full_join(zoo_log2FC_df[c(1, 10)], zoo_df, by = "X")
write.csv(zoo_avg_df, paste(Sys.Date(), "SpeciesMapped_SummaryCrossSpeciesExpression.csv"))
```
...
Species_DGEs <- llply(file.path(SpeciesMapped_DGE, sampleFiles_SpeciesMapped), reader)
...
bab_cts <- Species_DGEs[grep("baboon", names(Species_DGEs))] 

bonobo_cts <- Species_DGEs[grep("bonobo", names(Species_DGEs))]

orangutan_cts <- Species_DGEs[grep("orangutan", names(Species_DGEs))]

chimp_cts <- Species_DGEs[grep("chimpanzee", names(Species_DGEs))]

sqm_cts <- Species_DGEs[grep("squirrel", names(Species_DGEs))]

rhmac_cts <- Species_DGEs[grep("rhesus", names(Species_DGEs))]

ptmac_cts <- Species_DGEs[grep("pigtailed", names(Species_DGEs))]

gorilla_cts <- Species_DGEs[grep("gorilla", names(Species_DGEs))]
...
all_cts <- list(bonobo_cts, orangutan_cts, chimp_cts, gorilla_cts, sqm_cts, rhmac_cts, ptmac_cts, bab_cts)
names(all_cts) <- c("bonobo", "orangutan", "chimpanzee", "gorilla", "squirrel_monkey", "rhesus_macaque", "pigtailed_macaque", 
                    "olive_baboon")
...
all_DGE_limited <- all_DGE %>% llply(., function(x) x[c(1:4, 6:7)]) ##then select columns we care about (corresponds to X, SYMBOL, padj 
                                                ##and log2FoldChange for the NHP species and the related human file) for
                                        ##each data frame within the list

##Now rename the columns within the data frames in our list so that they make sense once we melt it down
##for plotting.
...
##Break apart all_species.m into the constituent melted species so we can then re-set the symbol order to what 
##we want based on what we did for all_DGE_ordered above. 
...
write.csv(common_species, file.path(output_dir, 
        paste(Sys.Date(), "DifferFromHuman_SpeciesMapped_CommonAllSpecies.csv")))

##Need a "copy" of 'all.m_species' to create a geom_text plot along with our heatmap later. This idea came from 
##https://github.com/tidyverse/ggplot2/issues/2656 and the rest of it was modified and implemented below in
##making the faceted heat maps. 
common_species_text <- common_species

##For every gene that is in our feature_human_innate data frame derived from InnateDB, we want to highlight them
##in our heatmap by making them in bold, italic face. 
common_species_text$faces <- ifelse((common_species_text$X %in% feature_human_innate$hsapiens_homolog_ensembl_gene),
                                   'bold.italic', 'plain') 

##Since we are making faceted plots, we only need one set of SYMBOLs shown per facet, so in our geom_text data source
##we will discard any of the rows for variables that have "related" in its name, i.e. getting rid of the second copy
##of symbols for a given species.
...
##Could not for the life of me find an easier way to do this, just joining each list of data frames for 
##each species so that you now have one data frame (instead of the original two) per species, containing
## the DGE for that species AND the DGE for the related human file. 
monkeys_joined <- list((monkeys_DGE[[1]] %>% purrr::reduce(full_join, by = "X")), (monkeys_DGE[[2]] %>% 
                            purrr::reduce(full_join, by = "X")), monkeys_DGE[[3]] %>%
                         purrr::reduce(full_join, by = "X"), monkeys_DGE[[4]] %>%
                         purrr::reduce(full_join, by = "X")) %>%
  llply(., function(x) x[c(1:4, 6:7)]) ##then select columns we care about (corresponds to X, SYMBOL, padj 
                                                ##and log2FoldChange for the NHP species and the related human file) for
                                        ##each data frame within the list
...
write.csv(monkeys, file.path(output_dir, paste(Sys.Date(), "DifferFromHuman_SpeciesMapped_AllMonkeys.csv")))

monkeys_text <- monkeys
monkeys_text$faces <- ifelse((monkeys_text$X %in% feature_human_innate$hsapiens_homolog_ensembl_gene),
                                   'bold.italic', 'plain') 
...
##each species so that you now have one data frame (instead of the original two) per species, containing
## the DGE for that species AND the DGE for the related human file. 
OWMs_joined <- list((OWMs_DGEs[[1]] %>% purrr::reduce(full_join, by = "X")), (OWMs_DGEs[[2]] %>% purrr::reduce(full_join, by = "X")),
          OWMs_DGEs[[3]] %>% purrr::reduce(full_join, by = "X")) %>%
  llply(., function(x) x[c(1:4, 6:7)]) ##then select columns we care about (corresponds to X, SYMBOL and 
                                      ##log2FoldChange for the NHP species and the related human file) for
                                        ##each data frame within the list
...
##Break apart OWMs.m into the constituent melted species so we can then re-set the symbol order to what 
##we want based on what we did for OWMs_ordered above. 


write.csv(OWMs_all, file.path(output_dir, paste(Sys.Date(), "DifferFromHuman_SpeciesMapped_OWMsOnly.csv")))

OWMs_all_text <- OWMs_all
OWMs_all_text$faces <- ifelse((OWMs_all_text$X %in% feature_human_innate$hsapiens_homolog_ensembl_gene), 'bold.italic', 'plain') 
...
species_DGE_ordered <- dplyr::arrange(species_DGE, desc(species_DGE[,5])) ##order by column containing
...
Na_Values_bonobo <- all_DGEs$bonobo[is.na(all_DGEs$bonobo$SYMBOL),] ##get positions of NA values in the SYMBOL column
levels(all_DGEs$bonobo$SYMBOL) <- c(levels(all_DGEs$bonobo$SYMBOL),"RNU1-125P")

all_DGEs$bonobo[34,2] = c("RNU1-125P")

Na_Values_baboon <- all_DGEs$olive_baboon[is.na(all_DGEs$olive_baboon$SYMBOL),] ##get positions of NA 
                        ##values in the SYMBOL column
levels(all_DGEs$olive_baboon$SYMBOL) <- c(levels(all_DGEs$olive_baboon$SYMBOL),"AL592183.1")

all_DGEs$olive_baboon[64,2] = c("AL592183.1")

Na_Values_gorilla <- all_DGEs$gorilla[is.na(all_DGEs$gorilla$SYMBOL),] ##get positions of NA values in
                                                        ##the SYMBOL column
levels(all_DGEs$gorilla$SYMBOL) <- c(levels(all_DGEs$gorilla$SYMBOL),"AC244197.3", "RF01233")

all_DGEs$gorilla[6,2] = c("AC244197.3")
all_DGEs$gorilla[55,2] = c("RF01233")

Na_Values_sqm <- all_DGEs$squirrel_monkey[is.na(all_DGEs$squirrel_monkey$SYMBOL),] ##get positions of 
                                              ##NA values in the SYMBOL column
levels(all_DGEs$squirrel_monkey$SYMBOL) <- c(levels(all_DGEs$squirrel_monkey$SYMBOL),"RF01986")

all_DGEs$squirrel_monkey[155,2] = c("RF01986")

Na_Values_orangutan <- all_DGEs$orangutan[is.na(all_DGEs$orangutan$SYMBOL),] ##get positions of NA 
                              ##values in the SYMBOL column
levels(all_DGEs$orangutan$SYMBOL) <- c(levels(all_DGEs$orangutan$SYMBOL),"AC025283.2")

all_DGEs$orangutan[38,2] = c("AC025283.2")

all_DGEs.m <- melt(all_DGEs)
write.csv(all_DGEs.m, file.path(output_dir,
                   paste(Sys.Date(),"DifferFromHuman_SpeciesMapped_EachSpeciesUnique.csv")))

##Break apart all_DGEs.m into the constituent melted species so we can then re-set the symbol order to
##what we want based on what we did for all_DGEs above. 
...
fewer$padj_species <- ifelse(grepl("related", fewer$variable), "", fewer$padj_species)
fewer$padj_human <- ifelse(!grepl("related", fewer$variable), "", fewer$padj_human)
...
pdf(file = file.path(output_dir, paste(Sys.Date(), "differFromHuman_shortList_SpeciesMapped.pdf")),
...
pdf(file = file.path(output_dir, 
                     paste(Sys.Date(), "differfromHuman_Monkeys_SpeciesMapped_unwrapped.pdf")), 
    height = 1.5, width = 5)
...
##group of species and no other) differentially expressed upon poly(I:C) treatment compared to human.
##Wrapped plot
pdf(file = file.path(output_dir, paste(Sys.Date(), "differfromHuman_Monkeys_SpeciesMapped.pdf")), 
    height = 4, width = 5)
...
fewer_baboon$variable <- with(fewer_baboon,
                        factor(variable,levels = c("olive_baboon", 
                                                   "human_related_olive_baboon ")))
pdf(file = file.path(output_dir, paste(Sys.Date(), "differfromHuman_olivebaboon_SpeciesMapped.pdf")),
    height =0.8, width = 15)
...
pdf(file = file.path(output_dir, paste(Sys.Date(), "differfromHuman_squirrelmonkey_SpeciesMapped.pdf")),
    height =0.8, width = 20)
...
##group of species and no other) differentially expressed upon poly(I:C) treatment compared to human.
##Wrapped plot
pdf(file = file.path(output_dir, paste(Sys.Date(), "differfromHuman_OWMs_SpeciesMapped.pdf")),
    height = 3, width = 5)
...
OWMs_all_filtered$variable <- with(OWMs_all_filtered,
                        factor(variable,levels = c("rhesus_macaque", 
                                                   "pigtailed_macaque", "olive_baboon", 
                                                   "human_related_olive_baboon ")))
pdf(file = file.path(output_dir, paste(Sys.Date(), "differfromHuman_OWMs_SpeciesMapped_unwrapped.pdf")),
    height = 1.2, width = 5)
...
pdf(file = file.path(output_dir, paste(Sys.Date(), 
      "differfromHuman_all_species_SpeciesMapped_unwrapped.pdf")), height = 1.5, width = 2.7)
...
##Wrapped plot
pdf(file = file.path(output_dir, paste(Sys.Date(), 
 "differfromHuman_all_species_SpeciesMapped_wrapped.pdf")), height = 8, width = 4)
...
list("pigtailed", "rhesus"), list("rhesus", "squirrel_monkey"),
list("pigtailed", "rhesus", "squirrel_monkey"), list("orangutan", "squirrel_monkey"),
list("chimp","bonobo"), list("orangutan", "pigtailed", "rhesus", "baboon", "squirrel_monkey"))

dot_colors <- c("purple", "darkgreen", "#bee1f4", "forestgreen", "#0088ce", 
 "#7dc3e8", "darkolivegreen3", "#00659c", "grey55", "orange", "orange", "grey55", "grey55", 
  "grey55", "grey55", "grey55", "orange")
...
boxplots <- boxplotter + 
 ...
 Matrix_layout <- UpSetR:::Create_layout(Matrix_setup, matrix.color, Matrix_col, matrix.dot.alpha)
  ...
  for(i in 1:8) {
      ...
      if(length(j) > 0) Matrix_layout$color[j] <- c("purple", "darkgreen", "forestgreen", "darkolivegreen3", "#bee1f4", "#0088ce", 
                   "#00659c", "#7dc3e8")[i]
  }
...
pdf(file.path(DGE_source, paste(Sys.Date(), "UpSetPlot_DGE_SpeciesMapped_vs_Humans.pdf")), width = 15, height = 7)
